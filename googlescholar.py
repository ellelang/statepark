...
driver = webdriver.Firefox(executable_path=r'C:\Users\langzx\geckodriver.exe')
...
def start_session(driver, query, n=0, patents=False, citations=False, start_year=2018, end_year=1950):
    ...
    while year >= end_year:
        ...
        finished = False
        while not finished:
            #soup = BeautifulSoup(driver.page_source,'html').encode("utf-8")
            soup = BeautifulSoup(driver.page_source, features ="lxml-xml")
            datalist = []  # empty list
            if soup.find(id="gs_captcha_ccl"):
                ...
                soup = BeautifulSoup(driver.page_source, features ="lxml-xml")
                #soup = BeautifulSoup(driver.page_source, features = 'html').encode("utf-8")
...
if __name__ == '__main__':
    ...
    # for any year
    start_session(driver, query="%22Wetland+Restoration+Cost%22",
